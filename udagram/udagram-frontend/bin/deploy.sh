aws s3 cp --recursive --acl public-read s3://udacity-udagram-tarek-aldahshan/
aws s3 cp --acl public-read --cache-control="max-age=0, no-cache, no-store, must-revalidate" s3://udacity-udagram-tarek-aldahshan/index.html
